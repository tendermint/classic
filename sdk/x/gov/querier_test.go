...
func getQueriedParams(t *testing.T, ctx sdk.Context, querier sdk.Querier) (DepositParams, VotingParams, TallyParams) {
	...
	err2 := amino.UnmarshalJSON(bz, &depositParams)
	require.Nil(t, err2)
	...
	err2 = amino.UnmarshalJSON(bz, &votingParams)
	require.Nil(t, err2)
	...
	err2 = amino.UnmarshalJSON(bz, &tallyParams)
	require.Nil(t, err2)

	return depositParams, votingParams, tallyParams
}

func getQueriedProposal(t *testing.T, ctx sdk.Context, querier sdk.Querier, proposalID uint64) Proposal {
	query := abci.RequestQuery{
		Path: strings.Join([]string{custom, QuerierRoute, QueryProposal}, "/"),
		Data: amino.MustMarshalJSON(NewQueryProposalParams(proposalID)),
	}
	...
	err2 := amino.UnmarshalJSON(bz, proposal)
	require.Nil(t, err2)
	return proposal
}

func getQueriedProposals(t *testing.T, ctx sdk.Context, querier sdk.Querier, depositor, voter sdk.AccAddress, status ProposalStatus, limit uint64) []Proposal {
	query := abci.RequestQuery{
		Path: strings.Join([]string{custom, QuerierRoute, QueryProposals}, "/"),
		Data: amino.MustMarshalJSON(NewQueryProposalsParams(status, limit, voter, depositor)),
	}
	...
	err2 := amino.UnmarshalJSON(bz, &proposals)
	require.Nil(t, err2)
	return proposals
}

func getQueriedDeposit(t *testing.T, ctx sdk.Context, querier sdk.Querier, proposalID uint64, depositor sdk.AccAddress) Deposit {
	query := abci.RequestQuery{
		Path: strings.Join([]string{custom, QuerierRoute, QueryDeposit}, "/"),
		Data: amino.MustMarshalJSON(NewQueryDepositParams(proposalID, depositor)),
	}
	...
	var deposit Deposit
	err2 := amino.UnmarshalJSON(bz, &deposit)
	require.Nil(t, err2)
	return deposit
}

func getQueriedDeposits(t *testing.T, ctx sdk.Context, querier sdk.Querier, proposalID uint64) []Deposit {
	query := abci.RequestQuery{
	...
	err2 := amino.UnmarshalJSON(bz, &deposits)
	require.Nil(t, err2)
	return deposits
}

func getQueriedVote(t *testing.T, ctx sdk.Context, querier sdk.Querier, proposalID uint64, voter sdk.AccAddress) Vote {
	query := abci.RequestQuery{
		Path: strings.Join([]string{custom, QuerierRoute, QueryVote}, "/"),
		Data: amino.MustMarshalJSON(NewQueryVoteParams(proposalID, voter)),
	}
	...
	var vote Vote
	err2 := amino.UnmarshalJSON(bz, &vote)
	require.Nil(t, err2)
	return vote
}

func getQueriedVotes(t *testing.T, ctx sdk.Context, querier sdk.Querier, proposalID uint64) []Vote {
	query := abci.RequestQuery{
		Path: strings.Join([]string{custom, QuerierRoute, QueryVote}, "/"),
		Data: amino.MustMarshalJSON(NewQueryProposalParams(proposalID)),
	}
	...
	var votes []Vote
	err2 := amino.UnmarshalJSON(bz, &votes)
	require.Nil(t, err2)
	return votes
}

func getQueriedTally(t *testing.T, ctx sdk.Context, querier sdk.Querier, proposalID uint64) TallyResult {
	query := abci.RequestQuery{
	...
	err2 := amino.UnmarshalJSON(bz, &tally)
	require.Nil(t, err2)
	return tally
}
...
func TestQueryParams(t *testing.T) {
	...
	getQueriedParams(t, ctx, querier)
}
...
func TestQueries(t *testing.T) {
	...
	depositParams, _, _ := getQueriedParams(t, ctx, querier)
	...
	amino.MustUnmarshalBinaryLengthPrefixed(res.Data, &proposalID1)
	...
	require.True(t, res.IsOK())
	amino.MustUnmarshalBinaryLengthPrefixed(res.Data, &proposalID2)
	...
	require.True(t, res.IsOK())
	amino.MustUnmarshalBinaryLengthPrefixed(res.Data, &proposalID3)
	...
	deposits := getQueriedDeposits(t, ctx, querier, proposalID1)
	require.Len(t, deposits, 1)
	deposit := getQueriedDeposit(t, ctx, querier, proposalID1, input.addrs[0])
	require.Equal(t, deposit, deposits[0])

	// check deposits on proposal2 match individual deposits
	deposits = getQueriedDeposits(t, ctx, querier, proposalID2)
	require.Len(t, deposits, 2)
	deposit = getQueriedDeposit(t, ctx, querier, proposalID2, input.addrs[0])
	require.True(t, deposit.Equals(deposits[0]))
	deposit = getQueriedDeposit(t, ctx, querier, proposalID2, input.addrs[1])
	require.True(t, deposit.Equals(deposits[1]))

	// check deposits on proposal3 match individual deposits
	deposits = getQueriedDeposits(t, ctx, querier, proposalID3)
	require.Len(t, deposits, 1)
	deposit = getQueriedDeposit(t, ctx, querier, proposalID3, input.addrs[1])
	require.Equal(t, deposit, deposits[0])

	// Only proposal #1 should be in Deposit Period
	proposals := getQueriedProposals(t, ctx, querier, nil, nil, StatusDepositPeriod, 0)
	require.Len(t, proposals, 1)
	require.Equal(t, proposalID1, proposals[0].ProposalID)

	// Only proposals #2 and #3 should be in Voting Period
	proposals = getQueriedProposals(t, ctx, querier, nil, nil, StatusVotingPeriod, 0)
	...
	proposals = getQueriedProposals(t, ctx, querier, nil, input.addrs[0], StatusNil, 0)
	require.Equal(t, proposalID2, (proposals[0]).ProposalID)
	require.Equal(t, proposalID3, (proposals[1]).ProposalID)

	// Test query votes on Proposal 2
	votes := getQueriedVotes(t, ctx, querier, proposalID2)
	require.Len(t, votes, 1)
	require.Equal(t, input.addrs[0], votes[0].Voter)

	vote := getQueriedVote(t, ctx, querier, proposalID2, input.addrs[0])
	require.Equal(t, vote, votes[0])

	// Test query votes on Proposal 3
	votes = getQueriedVotes(t, ctx, querier, proposalID3)
	...
	proposals = getQueriedProposals(t, ctx, querier, nil, nil, StatusNil, 0)
	require.Equal(t, proposalID1, (proposals[0]).ProposalID)
	...
	proposals = getQueriedProposals(t, ctx, querier, nil, input.addrs[1], StatusNil, 0)
	require.Equal(t, proposalID3, (proposals[0]).ProposalID)

	// Test query deposited by input.addrs[0]
	proposals = getQueriedProposals(t, ctx, querier, input.addrs[0], nil, StatusNil, 0)
	require.Equal(t, proposalID1, (proposals[0]).ProposalID)

	// Test query deposited by addr2
	proposals = getQueriedProposals(t, ctx, querier, input.addrs[1], nil, StatusNil, 0)
	require.Equal(t, proposalID2, (proposals[0]).ProposalID)
	require.Equal(t, proposalID3, (proposals[1]).ProposalID)

	// Test query voted AND deposited by addr1
	proposals = getQueriedProposals(t, ctx, querier, input.addrs[0], input.addrs[0], StatusNil, 0)
	require.Equal(t, proposalID2, (proposals[0]).ProposalID)
}
